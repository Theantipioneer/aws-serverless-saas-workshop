...
if [ "$IS_RUNNING_IN_EVENT_ENGINE" = false ]; then
  ...
  ADMIN_SITE_BUCKET=$(aws cloudformation describe-stacks --stack-name acumen-saas --query "Stacks[0].Outputs[?OutputKey=='AdminSiteBucket'].OutputValue" --output text)
...
if [[ $client -eq 1 ]]; then
  ...
  ADMIN_APPCLIENTID=$(aws cloudformation describe-stacks --stack-name acumen-saas --query "Stacks[0].Outputs[?OutputKey=='CognitoOperationUsersUserPoolClientId'].OutputValue" --output text)
  ADMIN_USERPOOL_ID=$(aws cloudformation describe-stacks --stack-name acumen-saas --query "Stacks[0].Outputs[?OutputKey=='CognitoOperationUsersUserPoolId'].OutputValue" --output text)
  ADMIN_USER_GROUP_NAME=$(aws cloudformation describe-stacks --stack-name acumen-saas --query "Stacks[0].Outputs[?OutputKey=='CognitoAdminUserGroupName'].OutputValue" --output text)
